 ...
 *
 *      node tools/build-island-geometry.mjs
 *
 *  ── The board is wwwroot/img/board.svg ──────────────────────────────────────
 ...
 *
 *  ── The layouts ─────────────────────────────────────────────────────────────
 *  Plain data: tools/layouts.json, in the island playground's own save format. To correct
 *  one, build it in the playground, save it, and paste the arrangement in — the placements
 *  and seams come from the same code that loads them back, so what you saw is what ships.
 *
...
const js = `// GENERATED by tools/build-island-geometry.mjs — do not edit by hand.
//
...
for (const [id, def] of Object.entries(layouts)) {
  ...
  const body = def.boards.map(b => {
    ...
    return `<g><path d="${d}" fill="#e2d6b4" stroke="#6b563d" stroke-width="1.2" stroke-linejoin="round"/>` +
           `<g transform="${local}" clip-path="url(#thumbClip)">${COAST_PATHS.map(c => `<path d="${c}" fill="none" stroke="#4a90c4" stroke-width="${COAST_BAND*2}" stroke-linejoin="round"/>`).join('')}</g></g>`;
  }).join('');
  fs.writeFileSync(path.join(THUMB_DIR, `${id}.svg`),
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${(x0-4).toFixed(1)} ${(y0-4).toFixed(1)} ${(x1-x0+8).toFixed(1)} ${(y1-y0+8).toFixed(1)}">` +
